...
module.exports = {
  ...
  getStartAndEndOfWeek : function(){
    let weekStart = weeksInMonth[0].start
    let weekEnd = weeksInMonth[0].start
  },
  makeMonthChartData : function(monthStudyData, weeksInMonth){
    ...
    let monthStudyTimeList_10ms = Array.from({length: weeksInMonth.length}, () => 0); 
    // 월의 공부기록을 주차별로 저장
    for(let i=0 ; i< monthStudyData.length; i++){
      ...
      while(weekIdx < weeksInMonth.length){
        ...
        if(weekStart.getTime() <= date && weekEnd.getTime() >= date){ // 현재 기록의 시작 시간이 해당 주차의 범위에 있는 경우
          monthStudyTimeList_10ms[weekIdx] += monthStudyData[i].studiedTime_10ms; // 공부 시간을 해당 주차의 기록에 저장 
          break;
        } else{
          weekIdx++;
        }
    ...
    let totalStudyTime_hour = 0;
    const monthStudyTimeList_hour = monthStudyTimeList_10ms.map((time) =>{
      const studyTime_hour = (time/(60*60*100)).toFixed(2); // 10ms -> 1hour
      totalStudyTime_hour += Number(studyTime_hour); // 총 공부시간에 더해줌
      return studyTime_hour;
    }) 
    return new CharDataForm(title, weeksInMonth_label, monthStudyTimeList_hour, totalStudyTime_hour);    
  },

  makeYearChartData : function(yearStudyData, monthsInYear){
